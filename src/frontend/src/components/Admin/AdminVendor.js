...
function AdminVendor(props){
    ...
    function showConfirm(vendor_id) {
        ...
        if (_confirm) {
            fetch(baseUrl + '/admin/delete-vendor/' + vendor_id + '/', {
                method: 'DELETE',
            })
            .then((response) => {
                                ...
                                <tbody>
                                    {
                                        VendorList.map((item, index) => 
                                            <tr>
                                                ...
                                                <td>{item.mobile}</td>
                                                <td>
                                                    <Link to={`/admin-page/vendor/${item.id}/${item.user.username}`} className='btn btn-primary btn-sm me-1'>Views   </Link>
